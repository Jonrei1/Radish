...
interface VitalsHistoryTableProps {
  ...
  onDelete: (vital: VitalSignRecord) => void;
  page: number;
  totalPages: number;
  total?: number;
            ...
            {sortedVitals.length === 0 ? (
              ...
              sortedVitals.map((v) => {
                ...
                const strikeClass = isGhost
                  ? 'line-through decoration-text-muted/65 decoration-1'
                  : '';
                ...
                return (
                  <tr
                    ...
                    className={`transition-all duration-200 ${
                      isGhost
                        ? 'opacity-55 grayscale blur-[0.5px] select-none hover:opacity-75 hover:blur-none'
                        : 'hover:bg-surface-2'
                    }`}
                  >
                    ...
                    <td className={`py-2.5 px-4 ${isGhost ? strikeClass : bpClass}`}>
                      {formatBloodPressure(v.sbp, v.dbp)}
                    </td>
                    <td className={`py-2.5 px-4 ${isGhost ? strikeClass : hrClass}`}>
                      {v.heartRate ?? '—'}
                    </td>
                    <td className={`py-2.5 px-4 ${isGhost ? strikeClass : rrClass}`}>
                      {v.respiratoryRate ?? '—'}
                    </td>
                    <td className={`py-2.5 px-4 ${isGhost ? strikeClass : tempClass}`}>
                      {formatTemperature(v.temperature)}
                    </td>
                    <td className={`py-2.5 px-4 ${isGhost ? strikeClass : spo2Class}`}>
                      ...
                      {v.measuredBySnapshot ? (
                        <div className="flex items-center gap-1.5">
                          <span className="truncate">
                            {v.measuredBySnapshot.lastName}, {v.measuredBySnapshot.firstName}
                          </span>
