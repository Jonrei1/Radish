...
export function formatBloodPressure(sbp: number | null | undefined, dbp: number | null | undefined): string {
  if (sbp == null && dbp == null) return '—/—';
  return `${sbp ?? '—'}/${dbp ?? '—'}`;
}

/**
 * Vital range classification for colored-status treatment.
 * Coarse clinical thresholds for visual triage only.
 */
...
export function classifyHeartRate(hr: number | null | undefined): 'normal' | 'warn' | 'critical' {
...
export function classifyOxygenSaturation(spo2: number | null | undefined): 'normal' | 'warn' | 'critical' {
...
export function classifyTemperature(
  temp: string | number | null | undefined
): 'normal' | 'warn' | 'critical' {
  if (temp == null || temp === '') return 'normal';
...
export function classifyRespiratoryRate(rr: number | null | undefined): 'normal' | 'warn' | 'critical' {
